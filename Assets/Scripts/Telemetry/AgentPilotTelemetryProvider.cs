...
{
    public class AgentPilotTelemetryProvider : BaseTelemetryProvider<AgentPilot, AgentPilotTelemetryProvider.AgentPilotTelemetry>
	{
		public class AgentPilotTelemetry : Telemetry.ChannelGroup
		{
			private AgentPilot agentPilot;
            ...
            {
				agentPilot = (AgentPilot)instance;
            ...
            public override void PollValues(float[] values, int index, Object instance)
            {
				...
				//Reference Info
				values[index + 5] = agentPilot.ReferenceSpeed;
				values[index + 6] = agentPilot.DeltaTime;
				values[index + 7] = agentPilot.ReferenceSample.throttle/100f;
				values[index + 8] = agentPilot.ReferenceSample.brake/100f;
				values[index + 9] = agentPilot.ReferenceSample.steeringAngle;
				values[index + 10] = agentPilot.ReferenceSample.drsPosition/100;

				//Status Info
				//we insert 1,-1 instead of 1,0 to be able to use conditional format
				//https://docs.microsoft.com/en-us/dotnet/standard/base-types/custom-numeric-format-strings
				values[index + 11] = agentPilot.IsOn ? 1f : -1f;
				values[index + 12] = agentPilot.IsStartup ? 1f : -1f;
			}
		}
	}
}
